<template>
        ...
        <span :class="{wrong: wrongs[index], right : rights[index], text: true}" >{{ letter }}</span>
        </span>
        </div>
...
let wrongs = ref(new Array(string.length).fill(false));
let rights = ref(new Array(string.length).fill(false));
let position = ref(0);
...
window.addEventListener("keydown", (ev) => {  
        ...
        if (specialCharacters.indexOf(ev.key) != -1) {
                ...
                if (position.value >= string.length) {
                        ...
                        (wrongs.value)[position.value] = false;
                        (rights.value)[position.value] = false;
                }
                ...
                (wrongs.value)[position.value] = true;
                (position.value)++;
        } 
        else {
                (rights.value)[position.value] = true;
                (position.value)++;
        }
})
