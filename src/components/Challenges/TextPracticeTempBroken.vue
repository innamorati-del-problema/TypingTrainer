<template>
  ...
  <div
    class="text-responsive m-4 mx-auto w-5/6 whitespace-nowrap rounded-xl bg-white p-3 text-center shadow-xl"
    @click="start"
  >
    <div v-if="!started" class="">
      <h1 class="absolute left-0 z-50 mt-2 w-screen text-4xl text-black">
        Clicca per iniziare
      </h1>
    </div>
    <div v-for="(row, i) in string">
      <span :class="{ blur: !started }" v-for="(letter, index) in row">
        <span
          :class="{
            'rounded-sm bg-[#ff00004d] text-[#858585] transition-colors':
              letterValues[index] == 1,
            'rounded-sm bg-[#42b5424b] text-[#858585] transition-colors':
              letterValues[index] == 3,
            'rounded-sm bg-[#fdfd184d] text-[#858585] transition-colors':
...
let i = 0;
let j = 0;
let n = 0; //getRndInteger(1,5);
for (i = 0; i < texts[n].text.length; i++) {
  for (j = 0; j < texts[n].text[i].length; j++) {
    string[i].push(texts[n].text[i][j]);
  }
}
console.log(string);
